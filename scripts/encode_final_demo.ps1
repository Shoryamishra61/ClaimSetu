...
$InputPath = Join-Path $ProjectRoot "output\video\claimsetu-final-submission.webm"
$OutputPath = Join-Path $ProjectRoot "output\video\claimsetu-final-submission.mp4"
$TemporaryPath = Join-Path $ProjectRoot "tmp\claimsetu-final-submission.mp4"
...
& $FfmpegPath -y -i $InputPath -t 116 -map 0:v:0 -map 0:a:0 `
